...
#define I2C_SLAVE_PACKAGE_SIZE 32 // amount of bytes that can be sent to the slave
#define I2C_SLAVE_BUFFER_SIZE 1512L

#define I2C_ORDER_OFFSET 5000L
#define I2C_REQUEST_OFFSET 10000L

#define I2C_BUFFER_SIZE I2C_SLAVE_BUFFER_SIZE + 200
...
uint16_t requestDataLength(); // get amount of the data available by the slave

uint16_t requestData();           // starts data transmission
uint16_t requestData(int amount); // starts data transmission
uint8_t requestDataPackage(int received_bytes, int package_size);
